...
public class TodoQuery {
	...
	static {//스태틱 블럭은 스태틱 필드를 초기화 하기 위해서 사용한다.
		...
		SELECT_TODO_ONE_BY_TODO_NO = "SELECT todo_no todoNo, todo_date todoDate, todo_title todoTitle, todo_content todoContent, todo_font_color todoFontColor FROM todo WHERE todo_no=? AND member_no=?";
		...
		SELECT_TODO_DDAY_LIST = "SELECT todo_no todoNo, todo_title todoTitle, todo_date todoDate, DATEDIFF(todo_date, DATE(NOW())) dday FROM todo WHERE todo_date > DATE(NOW()) AND member_no=? LIMIt 10";
	}
